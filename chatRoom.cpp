...
void Session::async_write(std::string messageBody, size_t messageLength){   // server to client
...
void accept_connection(boost::asio::io_context &io,char *port,tcp::acceptor &acceptor,Room &room,const tcp::endpoint& endpoint)
{
    ...
    acceptor.async_accept(handle_accept); // callback will be called after client connects 
}
